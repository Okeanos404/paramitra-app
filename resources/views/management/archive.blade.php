...
<div class="space-y-8">
    ...
    <div class="flex gap-4 border-b border-slate-200">
        <a href="{{ route('manajemen.arsip', ['type' => 'pemasukan']) }}" 
        ...
        <a href="{{ route('manajemen.arsip', ['type' => 'pengeluaran']) }}" 
    ...
    <div class="bg-white p-6 rounded-3xl border border-slate-100 shadow-xl shadow-slate-200/10">
        <form action="{{ route('manajemen.arsip') }}" method="GET" class="flex flex-wrap items-end gap-4">
            ...
            @if(request()->has('start_date'))
                <a href="{{ route('manajemen.arsip', ['type' => $type]) }}" class="px-6 py-2 bg-slate-100 text-slate-600 rounded-xl font-bold hover:bg-slate-200 transition-colors">Reset</a>
            @endif
        </form>
    </div>
